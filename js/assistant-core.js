...
(function () {
  ...
  // K.6 guardrails: this frontend layer never exposes private sources, invents legal citations,
  // or treats uploaded material as automatically authoritative.
  function safetyCheck(text) {
    if (/ignora (todas|las|tus) (reglas|instrucciones)|prompt injection|revela.*(prompt|instrucciones internas)|system prompt/.test(text))
      return "No puedo ignorar mis reglas de funcionamiento ni revelar instrucciones internas. Puedo ayudarte, en cambio, a formular la consulta profesional concreta.";
    if (/documentos? privados?|informacion confidencial|contraseñ|api key|clave secreta|datos de otros clientes/.test(text))
      return "No puedo revelar documentos privados, credenciales, datos confidenciales ni información de otros clientes. Si necesitas revisar un documento propio, la vía segura será habilitada mediante el sistema autorizado.";
    if (/dame una respuesta aunque no estes seguro|invent[a-z]*.*norma|cita.*aunque no exista/.test(text))
      return "No voy a inventar una norma, una fuente ni una certeza jurídica. Si la base documental no permite verificar el punto, debo indicarlo claramente.";
    return null;
  }

  function reply(input, config) {
    const text = normalize(input);
    const blocked = safetyCheck(text);
    if (blocked) return { reply:blocked, action:null, softLead:false, confidence:"blocked" };
    const st = state();
    const s = st.value;
    s.turns += 1;
    ...
    st.save();
    return {
      reply:"No tengo base suficiente para afirmar una respuesta específica sin riesgo de inventar o simplificar en exceso. Cuéntame un poco más: ¿tu consulta está relacionada con contabilidad, impuestos, finanzas, trabajadores, control, factibilidad o Finza.M?",
      action:null,
      softLead:false
    };
  }
